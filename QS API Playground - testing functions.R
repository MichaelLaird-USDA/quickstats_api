...
# Define the function to make API request for a given county and return a dataframe
fetch_county_data <- function(c_name) {
  params <- list(
    key = api_key,
    year__GE = 2022,
    source_desc = "CENSUS",
...
# List of the first 5 counties in Indiana alphabetically
counties <- c("ADAIR", "ADAMS", "ALLAMAKEE", "APPANOOSE", "AUDUBON")

# Initialize an empty dataframe
all_data <- data.frame()

# Cycle through the first 5 counties and fetch data
for (county in counties) {
  county_data <- fetch_county_data(county)
  if (!is.null(county_data)) {
    all_data <- bind_rows(all_data, county_data)
  }
}
